...
export const useCreatePost = () => {
  ...
  const createPostMutation = useMutation({
    mutationFn: async (postData: { content: string; imageUri?: string }) => {
      ...
      if (postData.imageUri) {
        ...
        const mimeTypeMap: Record<string, string> = {
          png: "image/png",
          jpg: "image/jpg",
          jpeg: "image/jpeg", // Add this line for JPEG file type: "image/png",
          gif: "image/gif",
          webp: "image/webp",
        };
    ...
    onSuccess: () => {
      ...
      Alert.alert("Success", "Post partager!");
    },
    onError: () => {
  ...
  const createPost = () => {
    if (!content.trim() && !selectedImage) {
      Alert.alert(" Post Vide", "Ecriver  Quelque chose ou ajouter une image avant  poster!");
      return;
    }
